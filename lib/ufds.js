    ...
    Object.keys(config.datacenters).forEach(function(datacenter){
        if(config.datacenters[datacenter].ufds){
            self.datacenters[datacenter] =
                new mod_ufds(config.datacenters[datacenter].ufds);
        }
    });
};
...
UFDS.prototype.getUserByEmail = function(email, callback){
    var self = this;
    ...
    Object.keys(self.datacenters).forEach(function(d){
        tasks.push(function(callback){
            self.datacenters[d].getUserByEmail(email, function(err, user){
                callback(err, user);
            });
        });
    });
